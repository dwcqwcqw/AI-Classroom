...
import type { QuizQuestion, InteractiveContent } from '@/lib/types/stage';
import type {
  Action,
  SpeechAction,
...
async function generateWidgetContent(
  ...
  return {
    ...
    widgetType: widgetType as NonNullable<InteractiveContent['widgetType']>,
    widgetConfig: (widgetConfig as unknown) as Record<string, unknown>,
    teacherActions: teacherActions as NonNullable<InteractiveContent['teacherActions']>,
  };
}
  ...
  if (outline.type === 'interactive' && 'html' in content) {
    const sceneResult = api.scene.create({
      ...
      content: {
        ...
        widgetType: content.widgetType as InteractiveContent['widgetType'],
        widgetConfig: content.widgetConfig as InteractiveContent['widgetConfig'],
        teacherActions: content.teacherActions as InteractiveContent['teacherActions'],
      },
      actions,
    });
