...
3. **Interactive scenes**: If a concept benefits from hands-on simulation/visualization, use `"type": "interactive"` with `widgetType` and `widgetOutline` fields (see system prompt for density).
...
5. **Quiz placement**: Insert **fewer** `quiz` scenes with **more** `questionCount` each (e.g. 6–12 questions per quiz for 考研-style practice), spaced every few slide blocks—avoid many single-question quiz pages
