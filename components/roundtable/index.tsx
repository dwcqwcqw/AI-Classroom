...
import { ParticipantsRailPanel } from '@/components/roundtable/participants-rail-panel';
...
export function Roundtable({
  ...
  const [mobileInteractionOpen, setMobileInteractionOpen] = useState(false);
  ...
  useEffect(() => {
    if (isPresenting) setMobileInteractionOpen(false);
  }, [isPresenting]);

  const discussionRequestId = discussionRequest?.id;
  useEffect(() => {
    if (isMdUp || !discussionRequestId) return;
    setMobileInteractionOpen(true);
  }, [isMdUp, discussionRequestId]);
  ...
  return (
    <div
      className={cn(
        'h-[160px] sm:h-[192px] w-full flex flex-col relative z-10 transition-all duration-300',
    ...
    >
      ...
      <div className="flex-1 flex items-stretch min-h-0">
        ...
        <div
          className={cn(
            'w-14 sm:w-[90px] shrink-0 flex flex-col border-r border-gray-100/50 dark:border-gray-700/50 bg-white/40 dark:bg-gray-900/40 overflow-visible relative transition-opacity duration-300',
            isPresenting && !controlsVisible && 'opacity-0 pointer-events-none',
          )}
        >
              ...
              <HoverCard openDelay={300} closeDelay={100}>
                <HoverCardTrigger asChild>
                  <div className="flex flex-col items-center gap-1">
                    <div
                      className={cn(
                        'relative w-9 h-9 sm:w-12 sm:h-12 rounded-full transition-all duration-500 flex items-center justify-center',
                    ...
                    >
                      ...
                      <div className="w-7 h-7 sm:w-10 sm:h-10 rounded-full bg-white dark:bg-gray-800 overflow-hidden relative z-10 shadow-sm border border-gray-50 dark:border-gray-700">
                        <img
                          src={teacherAvatar}
                          alt={teacherName}
                      ...
                      {activeRole === 'teacher' && (
                        <div className="absolute -right-0.5 top-0.5 w-3 h-3 sm:w-4 sm:h-4 bg-green-500 dark:bg-green-400 rounded-full border-2 border-white dark:border-gray-800 flex items-center justify-center z-20">
                    ...
                    <span
                      className={cn(
                        'max-w-[60px] sm:max-w-[80px] truncate px-1.5 sm:px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] font-bold tracking-wider uppercase border shadow-sm transition-all duration-300 bg-white/90 dark:bg-gray-800/90',
        ...
        <div className="flex-1 relative mx-1 sm:mx-2 md:mx-3 mb-2 min-w-0">
                ...
                >
                  ...
                  >
                    <div
                      ...
                      className={cn(
                        'relative px-3 sm:px-4 pt-2 pb-3 rounded-2xl text-[15px] leading-relaxed transition-all border w-full max-w-full md:w-[min(420px,calc(100%-3rem))] group/bubble flex flex-col max-h-[min(48vh,280px)] md:max-h-[110px]',
        ...
        {/* Right: Participants area (desktop) / bottom drawer on narrow viewports */}
        {isMdUp ? (
          <div
            className={cn(
              'w-[100px] sm:w-[140px] shrink-0 flex flex-col py-3 border-l border-gray-100/50 dark:border-gray-700/50 bg-gray-50/30 dark:bg-gray-900/30 overflow-visible transition-opacity duration-300',
              isPresenting && !controlsVisible && 'opacity-0 pointer-events-none',
            )}
          >
            <ParticipantsRailPanel
              ...
              discussionAnchorRef={discussionAnchorRef}
              speakingAgentId={speakingAgentId}
              thinkingState={thinkingState}
              getAgentConfig={getAgentConfig}
              t={t}
              engineMode={engineMode}
              onDiscussionSkip={onDiscussionSkip}
              onDiscussionStart={onDiscussionStart}
              onPlayPause={onPlayPause}
              isSendCooldown={isSendCooldown}
              ...
              onStudentAvatarRef={(id, el) => {
                if (el) studentAvatarRefs.current.set(id, el);
          ...
          <>
            <Dialog open={mobileInteractionOpen} onOpenChange={setMobileInteractionOpen}>
              <DialogContent
                showCloseButton
                className={cn(
                  'top-auto bottom-0 left-0 right-0 translate-x-0 translate-y-0 max-w-full w-full sm:max-w-full',
                  'max-h-[85vh] overflow-y-auto rounded-t-2xl rounded-b-none border-t p-4 pt-5 gap-3',
                )}
              >
                <DialogTitle className="text-base font-semibold pr-8">
                  {t('roundtable.mobileInteractionDrawerTitle')}
                </DialogTitle>
                <div className="flex flex-col min-h-[200px] border border-gray-100 dark:border-gray-800 rounded-xl bg-gray-50/30 dark:bg-gray-900/30 overflow-hidden">
                  <ParticipantsRailPanel
                    agentScrollRef={agentScrollRef}
                    studentParticipants={studentParticipants}
                    teacherParticipant={teacherParticipant}
            ...
            {!isPresenting && (
              <button
                type="button"
                className={cn(
                  'fixed z-40 flex items-center justify-center size-12 rounded-full',
                  'shadow-lg border border-purple-200/80 dark:border-purple-800 bg-white dark:bg-gray-900',
                ...
                aria-label={t('roundtable.mobileInteractionFab')}
                onClick={() => setMobileInteractionOpen((o) => !o)}
              >
